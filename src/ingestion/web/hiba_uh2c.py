# src/ingestion/web/scraper_uh2c_enhanced.py
"""
SCRAPER UNIVERSITAIRE - UH2C (amélioré)
Fusion du scraping ciblé (news, faculty) et du crawling BFS (exploration de pages, téléchargement d'assets)
Conserve la structure exacte du scraper uhc2  original.
"""
...
IMAGE_CACHE_FILE = "image_cache_uh2c.json"
...
SITES_CONFIG = {
    "fsjesm": {
        "name": "FSJES Mohammedia",
        "homepage": "https://fsjesm.ma",
        ...
        "max_pages": 20,          # Nombre max de pages HTML à explorer
    },
    "fsbm": {
        "name": "FSBM",
        "homepage": "https://www.fsbm.ma",
        "news_url": None,
        "faculty_url": None,
        "scrape_news": True,
        "scrape_faculty": True,
...
def save_raw_html(client, source_name: str, url: str, html_content: str, page_type: str) -> None:
...
def save_image(client, image_url: str, source_name: str, image_name: str = None) -> bool:
...
def save_document(client, document_url: str, source_name: str, document_name: str = None) -> bool:
...
def save_jsonld(client, source_name: str, url: str, json_data: dict, log_index: int, script_index: int, now: datetime) -> None:
...
def save_logs(client, source_name: str, logs: list) -> None:
...
def save_structured_data(client, source_name: str, data_type: str, data_list: list) -> int:
...
def save_consolidated_data(client, all_news: list, all_faculty: list) -> None:
    """Version inchangée."""
    try:
        partition = get_date_partition()
        timestamp = partition["timestamp"]
...
def save_stats(client, source_name: str, stats: dict) -> None:
    """Version inchangée."""
    try:
        partition = get_date_partition()
        timestamp = partition["timestamp"]
...
# --- FSJESM ---
def scrape_fsjesm_news(client, url: str, session: requests.Session) -> list:
    logger.info(f"  Scraping FSJESM actualités from: {url}")
    news_list = []
    try:
        response = safe_request(session, url)
        if not response:
            return []
        save_raw_html(client, "fsjesm", url, response.text, "news")
        soup = BeautifulSoup(response.text, 'html.parser')
        ...
        for article in articles:
            ...
            if img_elem and img_elem.get('src'):
                ...
                if is_valid_url(image_url):
                    save_image(client, image_url, "fsjesm", f"fsjesm_news_{hashlib.md5(image_url.encode()).hexdigest()[:8]}")
            doc_urls = []
            for link in article.find_all('a', href=True):
                ...
                if full_url.lower().endswith(('.pdf','.doc','.docx','.xls','.xlsx','.ppt','.pptx','.zip')):
                    ...
                    save_document(client, full_url, "fsjesm")
            news_list.append({
                "title": title,
                "url": url_article,
...
def scrape_fsjesm_faculty(client, url: str, session: requests.Session) -> list:
    logger.info(f"  Scraping FSJESM faculty from: {url}")
    faculty_list = []
    try:
        response = safe_request(session, url)
        if not response:
            return []
        save_raw_html(client, "fsjesm", url, response.text, "faculty")
        soup = BeautifulSoup(response.text, 'html.parser')
...
# --- FSBM ---
def scrape_fsbm_news(client, url: str, session: requests.Session) -> list:
    logger.info(f"  Scraping FSBM actualités from: {url}")
    news_list = []
    try:
        response = safe_request(session, url)
        if not response:
            return []
        save_raw_html(client, "fsbm", url, response.text, "news")
        soup = BeautifulSoup(response.text, 'html.parser')
        ...
        for article in articles:
            ...
            if img_elem and img_elem.get('src'):
                ...
                image_url = urljoin(url, img_src)
                if is_valid_url(image_url):
                    save_image(client, image_url, "fsbm", f"fsbm_news_{hashlib.md5(image_url.encode()).hexdigest()[:8]}")
            doc_urls = []
            for link in article.find_all('a', href=True):
                href = link['href']
                full_url = urljoin(url, href)
                if full_url.lower().endswith(('.pdf','.doc','.docx','.xls','.xlsx','.ppt','.pptx','.zip')):
                    doc_urls.append(full_url)
                    save_document(client, full_url, "fsbm")
            news_list.append({
                "title": title,
                "url": url_article,
                "publication_date": date_text,
...
def scrape_fsbm_faculty(client, url: str, session: requests.Session) -> list:
    logger.info(f"  Scraping FSBM faculty from: {url}")
    faculty_list = []
    try:
        response = safe_request(session, url)
        if not response:
            return []
        save_raw_html(client, "fsbm", url, response.text, "faculty")
        soup = BeautifulSoup(response.text, 'html.parser')
        email_pattern = re.compile(r'[\w\.-]+@[\w\.-]+\.\w+')
...
# --- ENCGCASA ---
def scrape_encgcasa_news(client, url: str, session: requests.Session) -> list:
    logger.info(f"  Scraping ENCGCASA actualités from: {url}")
    news_list = []
    try:
        response = safe_request(session, url)
        if not response:
            return []
        save_raw_html(client, "encgcasa", url, response.text, "news")
        soup = BeautifulSoup(response.text, 'html.parser')
        articles = soup.find_all('article', class_=re.compile(r'node-?article|post|entry'))
        if not articles:
            articles = soup.find_all('div', class_='views-row')
        for article in articles:
            ...
            if img_elem and img_elem.get('src'):
                ...
                image_url = urljoin(url, img_src)
                if is_valid_url(image_url):
                    save_image(client, image_url, "encgcasa", f"encgcasa_news_{hashlib.md5(image_url.encode()).hexdigest()[:8]}")
            doc_urls = []
            for link in article.find_all('a', href=True):
                href = link['href']
                full_url = urljoin(url, href)
                if full_url.lower().endswith(('.pdf','.doc','.docx','.xls','.xlsx','.ppt','.pptx','.zip')):
                    doc_urls.append(full_url)
                    save_document(client, full_url, "encgcasa")
            news_list.append({
                "title": title,
                "url": url_article,
                "publication_date": date_text,
                "image_url": image_url if is_valid_url(image_url) else "",
                "category": "Actualités",
                "source": "ENCG Casablanca",
                "institution": "ENCGCASA",
                "documents": doc_urls
            })
        logger.info(f"      ✅ {len(news_list)} actualités trouvées")
    except Exception as e:
...
def scrape_encgcasa_faculty(client, url: str, session: requests.Session) -> list:
    logger.info(f"  Scraping ENCGCASA faculty from: {url}")
    faculty_list = []
    try:
        response = safe_request(session, url)
        if not response:
            return []
        save_raw_html(client, "encgcasa", url, response.text, "faculty")
        soup = BeautifulSoup(response.text, 'html.parser')
        email_pattern = re.compile(r'[\w\.-]+@[\w\.-]+\.\w+')
        rows = soup.find_all('tr')
        if not rows:
...
# --- ENSCASA ---
def scrape_enscasa_news(client, url: str, session: requests.Session) -> list:
    logger.info(f"  Scraping ENSCASA actualités from: {url}")
    news_list = []
    try:
        response = safe_request(session, url)
        if not response:
            return []
        save_raw_html(client, "enscasa", url, response.text, "news")
        soup = BeautifulSoup(response.text, 'html.parser')
        articles = soup.find_all('article', class_=re.compile(r'node-?article|post|entry'))
        if not articles:
            articles = soup.find_all('div', class_='views-row')
        for article in articles:
            ...
            if img_elem and img_elem.get('src'):
                ...
                image_url = urljoin(url, img_src)
                if is_valid_url(image_url):
                    save_image(client, image_url, "enscasa", f"enscasa_news_{hashlib.md5(image_url.encode()).hexdigest()[:8]}")
            doc_urls = []
            for link in article.find_all('a', href=True):
                href = link['href']
                full_url = urljoin(url, href)
                if full_url.lower().endswith(('.pdf','.doc','.docx','.xls','.xlsx','.ppt','.pptx','.zip')):
                    doc_urls.append(full_url)
                    save_document(client, full_url, "enscasa")
            news_list.append({
                "title": title,
                "url": url_article,
                "publication_date": date_text,
...
def scrape_enscasa_faculty(client, url: str, session: requests.Session) -> list:
    logger.info(f"  Scraping ENSCASA faculty from: {url}")
    faculty_list = []
    try:
        response = safe_request(session, url)
        if not response:
            return []
        save_raw_html(client, "enscasa", url, response.text, "faculty")
        soup = BeautifulSoup(response.text, 'html.parser')
        email_pattern = re.compile(r'[\w\.-]+@[\w\.-]+\.\w+')
        rows = soup.find_all('tr')
        if not rows:
...
def crawl_faculty_bfs(client, faculty_name: str, base_url: str, max_pages: int = 500) -> None:
    """
    ...
    with requests.Session() as session:
        ...
        while queue and len(visited_pages) < max_pages:
            ...
            try:
                ...
                save_raw_html(client, faculty_name, url, response.text, "crawl_bfs")
                ...
                for script_idx, script in enumerate(scripts):
                    ...
                    try:
                        ...
                        save_jsonld(client, faculty_name, url, json_data, len(logs), script_idx, now)
                    except Exception:
                            ...
                            try:
                                with session.get(full_url, timeout=30, verify=False, stream=True) as file_res:
                                    if "text/html" not in file_res.headers.get("Content-Type", "").lower() and file_res.status_code == 200:
                                        save_document(client, full_url, faculty_name)
                                    else:
                        ...
                        if full_url not in downloaded_assets and full_url not in visited_pages:
                            ...
                            try:
                                ...
                                if img_res.status_code == 200:
                                    save_image(client, full_url, faculty_name)
                            except Exception as e:
    ...
    save_logs(client, faculty_name, logs)
...
def run():
    ...
    partition = get_date_partition()
    logger.info("="*70)
    logger.info("🚀 SCRAPER UNIVERSITAIRE - UH2C (amélioré)")
    logger.info("="*70)
    ...
    for site_key, site_config in SITES_CONFIG.items():
        ...
        if homepage_response:
            save_raw_html(client, site_key, site_config["homepage"], homepage_response.text, "homepage")
        ...
        if site_config.get("scrape_news", True) and site_config.get("news_url"):
            ...
            try:
                if site_key == "fsjesm":
                    news = scrape_fsjesm_news(client, site_config["news_url"], session)
                elif site_key == "fsbm":
                    news = scrape_fsbm_news(client, site_config["news_url"], session)
                elif site_key == "enscasa":
                    news = scrape_enscasa_news(client, site_config["news_url"], session)
                elif site_key == "encgcasa":
                    news = scrape_encgcasa_news(client, site_config["news_url"], session)
                else:
                    news = []
                if news:
                    source_news.extend(news)
                    saved = save_structured_data(client, site_key, "news", source_news)
        ...
        if site_config.get("scrape_faculty", True) and site_config.get("faculty_url"):
            ...
            try:
                if site_key == "fsjesm":
                    faculty = scrape_fsjesm_faculty(client, site_config["faculty_url"], session)
                elif site_key == "fsbm":
                    faculty = scrape_fsbm_faculty(client, site_config["faculty_url"], session)
                elif site_key == "enscasa":
                    faculty = scrape_enscasa_faculty(client, site_config["faculty_url"], session)
                elif site_key == "encgcasa":
                    faculty = scrape_encgcasa_faculty(client, site_config["faculty_url"], session)
                else:
                    faculty = []
                if faculty:
                    source_faculty.extend(faculty)
                    saved = save_structured_data(client, site_key, "faculty", source_faculty)
        ...
        if site_config.get("crawl_mode", False):
            ...
            try:
                ...
                crawl_faculty_bfs(client, site_key, site_config["homepage"], max_pages)
        ...
        # ---- 4. Statistiques ----
        save_stats(client, site_key, stats)
    ...
    logger.info("="*70)
    save_consolidated_data(client, all_news, all_faculty)

    # ---- Résumé ----
    logger.info("\n" + "="*70)
    logger.info("📊 RÉSUMÉ FINAL - UH2C")
    logger.info("="*70)
