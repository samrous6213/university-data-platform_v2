...
# Modification 1 : On utilise TON client MinIO
from src.storage.minio.ayoub_client import MinIOClient

# Modification 2 : On cible l'USMBA avec la bonne redirection
...
def _is_allowed_url(url: str) -> bool:
    ...
    # Modification 3 : On bloque le robot sur le domaine usmba.ac.ma
    return host.endswith("usmba.ac.ma")
...
def crawl_usmba():
    ...
    # Modification 4 : On change le nom du robot
    session.headers.update({
        "User-Agent": "Mozilla/5.0 (compatible; USMBA crawler/1.0; +https://www.usmba.ac.ma)",
    })
    ...
    # Upload du log final dans le bon dossier USMBA
    client.upload_json(
        bucket_name="data-lake",
        object_name=f"raw/logs/usmba/crawl_{now.strftime('%Y%m%d_%H%M%S')}.json",
        data=logs,
    )

    print(f"Scraping USMBA terminé : {len(visited)} pages récupérées.")
