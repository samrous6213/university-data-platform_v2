...
def build_session(max_retries: int = 5) -> requests.Session:
    """Session avec retry/backoff sur erreurs transitoires (API CKAN + téléchargement fichiers)."""
...
def fetch_package(session: requests.Session, package_id: str) -> dict:
    """Appelle package_show (CKAN) : métadonnées du dataset + liste de ses ressources."""
...
def ingest_dataset(client: MinIOClient, session: requests.Session, package_id: str) -> dict:
    ...
    try:
        ...
        # Traçabilité : stockage des métadonnées du dataset (raw-json)
        package_object_name = (
        ...
        client.upload_json(
            ...
            metadata={
                ...
                "source_url": f"{CKAN_BASE_URL}/package_show?id={package_id}",
        ...
        for resource in resources:
            ...
            if not resource_url:
                logger.warning("Ressource sans URL, ignorée : dataset=%s resource=%s", package_id, resource_id)
                continue
            ...
            try:
                ...
                # Affectation claire du bucket selon le format de la ressource
                ...
                raw_object_path = f"s3://{bucket}/{object_name}"
                ...
                logger.info("Ressource stockée : %s (format=%s)", raw_object_path, resource_format)
        ...
        logger.info("Log stocké : s3://%s/%s", LOG_BUCKET, log_object_name)
